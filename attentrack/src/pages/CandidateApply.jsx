...
export default function CandidateApply() {
  ...
  if (isLoadingJob) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${darkMode ? 'bg-[#090d16] text-slate-100' : 'bg-slate-100 text-slate-900'}`}>
  ...
  return (
    <div className={darkMode ? 'min-h-screen bg-[#07090f] text-slate-100 font-sans transition-colors duration-300' : 'min-h-screen bg-[#f1f5f9] text-slate-900 font-sans transition-colors duration-300'}>

      {/* TOP BRANDING BAR */}
      <header className={darkMode ? 'sticky top-0 z-40 border-b bg-[#0d1117]/90 border-purple-900/30 backdrop-blur-xl shadow-lg shadow-purple-900/10' : 'sticky top-0 z-40 border-b bg-background/95 border-slate-200 shadow-xs backdrop-blur-md'}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className={darkMode ? 'text-sm font-extrabold text-slate-100' : 'text-sm font-extrabold text-slate-900'}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className={darkMode ? 'hidden sm:flex items-center p-1 rounded-xl border text-xs font-bold bg-[#0f172a] border-purple-800/30' : 'hidden sm:flex items-center p-1 rounded-xl border text-xs font-bold bg-slate-100 border-slate-200'}>
              <button
                type="button"
                onClick={() => setViewMode('wizard')}
                className={`px-3 py-1.5 rounded-lg transition-all cursor-pointer ${viewMode === 'wizard' ? 'bg-purple-600 text-foreground shadow-sm font-black' : darkMode ? 'text-slate-400 hover:text-slate-100' : 'text-slate-600 hover:text-slate-900'}`}
              >
                Wizard Mode
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-all cursor-pointer ${viewMode === 'full' ? 'bg-purple-600 text-foreground shadow-sm font-black' : darkMode ? 'text-slate-400 hover:text-slate-100' : 'text-slate-600 hover:text-slate-900'}`}
              >
                Single Page
              </button>
            </div>
            ...
            <button
              ...
              className={darkMode ? 'p-2.5 rounded-xl border text-xs font-bold transition-all flex items-center gap-2 cursor-pointer bg-white/5 border-white/10 text-slate-300 hover:bg-white/10' : 'p-2.5 rounded-xl border text-xs font-bold transition-all flex items-center gap-2 cursor-pointer bg-background border-slate-200 text-slate-700 hover:bg-slate-50 shadow-xs'}
            >
            ...
            <button
              ...
              className={darkMode ? 'p-2.5 rounded-xl border transition-all cursor-pointer bg-white/5 border-white/10 text-amber-400 hover:bg-amber-500/10' : 'p-2.5 rounded-xl border transition-all cursor-pointer bg-background border-slate-200 text-slate-700 hover:bg-slate-50 shadow-xs'}
        ...
        {submitted ? (
          ...
          <div className={darkMode ? 'max-w-2xl mx-auto rounded-3xl p-8 md:p-12 shadow-2xl space-y-6 text-center animate-in zoom-in-95 border bg-[#1e293b] border-purple-800/30 text-slate-100' : 'max-w-2xl mx-auto rounded-3xl p-8 md:p-12 shadow-md space-y-6 text-center animate-in zoom-in-95 border bg-background border-slate-200 text-slate-900'}>
            ...
            <div className="space-y-2">
              ...
              <p className={darkMode ? 'text-sm text-slate-100 max-w-md mx-auto leading-relaxed' : 'text-sm text-slate-600 max-w-md mx-auto leading-relaxed'}>
                Thank you, <strong className="text-purple-500 font-extrabold">{formData.fullName}</strong>. Your application for <strong>{job?.title}</strong> has been saved directly into our ATS recruitment pipeline. Your resume was securely uploaded to our cloud.
              </p>
            </div>

            <div className={darkMode ? 'p-5 bg-[#0f172a] border border-purple-800/30 rounded-2xl text-xs space-y-2 max-w-md mx-auto text-left font-mono text-slate-300' : 'p-5 bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-2 max-w-md mx-auto text-left font-mono text-slate-800'}>
            ...
            {/* ONE SUBMISSION ONLY NOTICE */}
            <div className={darkMode ? 'flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/25 text-amber-300 text-xs font-semibold max-w-md mx-auto' : 'flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-700 text-xs font-semibold max-w-md mx-auto'}>
              <ShieldCheck className="w-4 h-4 shrink-0" />
              <span>Each candidate can only submit <strong>one application</strong> per role. This tab can now be closed.</span>
            </div>

            <button
              type="button"
              onClick={() => window.close()}
              className="inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-xl font-bold text-xs shadow-lg shadow-purple-500/30 transition-all cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 lg:sticky lg:top-24 space-y-6">
              <div className={darkMode ? 'rounded-3xl overflow-hidden shadow-2xl shadow-purple-900/20 border bg-gradient-to-b from-[#1a1f35] to-[#141928] border-purple-700/20' : 'rounded-3xl overflow-hidden shadow-md border bg-background border-slate-200'}>
                ...
                <div className="p-6 md:p-7 space-y-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={darkMode ? 'text-xs font-mono text-slate-400 font-bold' : 'text-xs font-mono text-muted-foreground font-bold'}>
                      {job?.id || 'JR-204'}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <h1 className={darkMode ? 'text-2xl md:text-3xl font-black text-slate-100 tracking-tight leading-snug' : 'text-2xl md:text-3xl font-black text-slate-900 tracking-tight leading-snug'}>
                      {job?.title}
                    </h1>
                    <p className={darkMode ? 'text-xs text-slate-400 mt-1 flex items-center gap-1.5 font-semibold' : 'text-xs text-slate-600 mt-1 flex items-center gap-1.5 font-semibold'}>
                  ...
                  <div className="space-y-2.5 text-xs font-bold">
                    <div className={darkMode ? 'flex items-center gap-2.5 bg-white/5 backdrop-blur-sm border border-white/10 text-slate-200 p-3 rounded-2xl' : 'flex items-center gap-2.5 bg-slate-100 border border-slate-200 text-slate-900 p-3 rounded-2xl'}>
                  ...
                  {/* JOB OVERVIEW */}
                  <div className={darkMode ? 'pt-4 border-t border-purple-800/30/80 space-y-3.5 text-xs text-slate-100 leading-relaxed' : 'pt-4 border-t border-slate-200 space-y-3.5 text-xs text-slate-700 leading-relaxed'}>
                    <div>
                      <h4 className={darkMode ? 'font-black text-slate-100 uppercase tracking-wider text-[11px] mb-1' : 'font-black text-slate-900 uppercase tracking-wider text-[11px] mb-1'}>
                        About the Role
                      </h4>
                    ...
                    {job?.requirements && (
                      <div>
                        ...
                        <div className={darkMode ? 'whitespace-pre-line font-mono bg-[#0d1020] p-3.5 rounded-2xl border border-purple-800/30 text-slate-300 max-h-44 overflow-y-auto' : 'whitespace-pre-line font-mono bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-slate-800 max-h-44 overflow-y-auto'}>
                          {job.requirements}
                        </div>
                      </div>
                    )}
                  ...
                  {viewMode === 'wizard' && (
                    <div className={darkMode ? 'pt-4 border-t border-purple-800/30/80 space-y-2' : 'pt-4 border-t border-slate-200 space-y-2'}>
                      <div className="flex items-center justify-between text-xs font-black">
                        <span className={darkMode ? 'text-slate-400 uppercase tracking-wider text-[10px]' : 'text-muted-foreground uppercase tracking-wider text-[10px]'}>Progress</span>
                        <span className="text-purple-500 font-mono">{Math.round((currentStep / totalSteps) * 100)}%</span>
                      </div>
                      <div className={darkMode ? 'h-2.5 w-full bg-[#0d1020] rounded-full overflow-hidden border border-purple-800/30 p-0.5' : 'h-2.5 w-full bg-slate-200 rounded-full overflow-hidden p-0.5'}>
                        <div
            ...
            <div className="lg:col-span-8 space-y-6">
                    ...
                    <button
                      ...
                      className={`flex-1 shrink-0 whitespace-nowrap flex items-center justify-center gap-2 py-3 px-3 rounded-xl transition-all cursor-pointer min-w-[100px] sm:min-w-[120px] ${
                        currentStep === st.num
                          ...
                          : darkMode ? 'text-slate-100 hover:text-slate-100' : 'text-slate-600 hover:text-slate-900'
                      }`}
                    >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {(viewMode === 'full' || currentStep === 1) && (
                  <div className={darkMode ? 'rounded-3xl p-6 md:p-8 space-y-5 border shadow-2xl shadow-purple-900/15 bg-gradient-to-br from-[#161b2e] to-[#111523] border-purple-700/20' : 'rounded-3xl p-6 md:p-8 space-y-5 border shadow-md bg-background border-slate-200'}>
                    <div className={darkMode ? 'flex items-center justify-between border-b pb-4 border-purple-800/30' : 'flex items-center justify-between border-b pb-4 border-slate-200'}>
                      ...
                      <span className={darkMode ? 'text-xs text-slate-400 font-bold' : 'text-xs text-muted-foreground font-bold'}>* Mandatory</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-1">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={darkMode ? 'w-full px-4 py-3.5 rounded-2xl text-sm border bg-[#0d1020] border-purple-800/30 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/25 transition-all font-semibold shadow-inner' : 'w-full px-4 py-3.5 rounded-2xl text-sm border bg-slate-50 border-slate-300 text-slate-900 placeholder:text-muted-foreground focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-600/20 transition-all font-semibold'}
                        />
                      </div>
                ...
                {(viewMode === 'full' || currentStep === 3) && (
                  <div className={darkMode ? 'rounded-3xl p-6 md:p-8 space-y-5 border shadow-2xl shadow-purple-900/15 bg-gradient-to-br from-[#161b2e] to-[#111523] border-purple-700/20' : 'rounded-3xl p-6 md:p-8 space-y-5 border shadow-md bg-background border-slate-200'}>
                    <div className={darkMode ? 'flex items-center justify-between border-b pb-4 border-purple-800/30' : 'flex items-center justify-between border-b pb-4 border-slate-200'}>
                      ...
                      <span className={darkMode ? 'text-xs text-slate-400 font-bold' : 'text-xs text-muted-foreground font-bold'}>PDF, DOC, DOCX (Max 10MB)</span>
                    </div>
                    ...
                    <div
                      ...
                      className={darkMode ? 'border-2 border-dashed rounded-3xl p-8 text-center cursor-pointer transition-all border-purple-800/40 bg-[#0d1020] hover:border-purple-500 hover:bg-purple-950/20' : 'border-2 border-dashed rounded-3xl p-8 text-center cursor-pointer transition-all border-slate-300 bg-slate-50 hover:border-purple-600'}
                    >
                      <input
                        ref={fileInputRef}
                        type="file"
                      ...
                      {base64File ? (
                        <div className="space-y-3">
                          ...
                          <div>
                            <div className={darkMode ? 'text-sm font-black text-slate-100' : 'text-sm font-black text-slate-900'}>{fileName}</div>
                        ...
                        <div className="space-y-2.5">
                          ...
                          <div className={darkMode ? 'text-sm font-black text-slate-100' : 'text-sm font-black text-slate-900'}>
                            Drag & Drop Resume here, or <span className="text-purple-500 underline">Browse File</span>
                          </div>
                          <p className={darkMode ? 'text-xs text-slate-400 max-w-xs mx-auto' : 'text-xs text-slate-600 max-w-xs mx-auto'}>
                            Securely uploaded to our cloud storage. Max 10MB.
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                      <div>
                        <label className={darkMode ? 'block text-xs font-black text-slate-300 uppercase tracking-wider mb-2 flex items-center gap-1' : 'block text-xs font-black text-slate-800 uppercase tracking-wider mb-2 flex items-center gap-1'}>
                        ...
                        <input
                          ...
                          className={darkMode ? 'w-full px-3.5 py-3 rounded-2xl text-xs border bg-[#0d1020] border-purple-800/30 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/25 font-semibold shadow-inner' : 'w-full px-3.5 py-3 rounded-2xl text-xs border bg-slate-50 border-slate-300 text-slate-900 placeholder:text-muted-foreground focus:outline-none focus:border-purple-600 font-semibold'}
                        />
                      </div>
                ...
                {(viewMode === 'full' || currentStep === 4) && (
                  <div className={darkMode ? 'rounded-3xl p-6 md:p-8 space-y-5 border shadow-2xl shadow-purple-900/15 bg-gradient-to-br from-[#161b2e] to-[#111523] border-purple-700/20' : 'rounded-3xl p-6 md:p-8 space-y-5 border shadow-md bg-background border-slate-200'}>
                    <div className={darkMode ? 'flex items-center justify-between border-b pb-4 border-purple-800/30' : 'flex items-center justify-between border-b pb-4 border-slate-200'}>
                      ...
                      <span className={darkMode ? 'text-xs text-slate-400 font-bold' : 'text-xs text-muted-foreground font-bold'}>Final Step</span>
                    </div>

                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <label className={darkMode ? 'block text-xs font-black text-slate-300 uppercase tracking-wider' : 'block text-xs font-black text-slate-800 uppercase tracking-wider'}>
                      ...
                      <textarea
                        ...
                        className={darkMode ? 'w-full px-4 py-3.5 rounded-2xl text-sm border bg-[#0d1020] border-purple-800/30 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/25 transition-all resize-none font-semibold shadow-inner' : 'w-full px-4 py-3.5 rounded-2xl text-sm border bg-slate-50 border-slate-300 text-slate-900 placeholder:text-muted-foreground focus:outline-none focus:border-purple-600 transition-all resize-none font-semibold'}
                      />
                    </div>

                    {/* SUMMARY REVIEW CARD */}
                    <div className={darkMode ? 'p-4 rounded-2xl bg-[#0d1020] border border-purple-800/30 space-y-2 text-xs text-slate-300' : 'p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2 text-xs text-slate-800'}>
                      <h4 className="font-black text-purple-500 uppercase tracking-wider text-[11px] mb-2">Application Summary Review</h4>
                      <div className="grid grid-cols-2 gap-3">
                        <div>Applicant: <strong className={darkMode ? 'text-slate-100' : 'text-slate-900'}>{formData.fullName || 'Not specified'}</strong></div>
                        <div>Email: <strong className={darkMode ? 'text-slate-100' : 'text-slate-900'}>{formData.email || 'Not specified'}</strong></div>
                        <div>Phone: <strong className={darkMode ? 'text-slate-100' : 'text-slate-900'}>{formData.phone || 'Not specified'}</strong></div>
                        <div>Qualification: <strong className={darkMode ? 'text-slate-100' : 'text-slate-900'}>{formData.highestQualification}</strong></div>
                        <div>Experience: <strong className={darkMode ? 'text-slate-100' : 'text-slate-900'}>{formData.experience || 'Not specified'}</strong></div>
                ...
                {viewMode === 'wizard' ? (
                  <div className="flex items-center justify-between pt-2 pb-12">
                    {currentStep > 1 ? (
                      <button
                        ...
                        className={darkMode ? 'px-6 py-3.5 rounded-2xl border font-black text-xs transition-all flex items-center gap-2 cursor-pointer bg-white/5 border-white/10 text-slate-300 hover:bg-white/10 hover:border-purple-500/40' : 'px-6 py-3.5 rounded-2xl border font-black text-xs transition-all flex items-center gap-2 cursor-pointer bg-background border-slate-300 text-slate-800 hover:bg-slate-100'}
                      >
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 pb-12">
                    ...
                    <div className={darkMode ? 'flex items-center gap-2 text-xs text-slate-400 font-bold' : 'flex items-center gap-2 text-xs text-slate-600 font-bold'}>
                      <ShieldCheck className="w-4 h-4 text-purple-500" />
                      <span>Encrypted SSL • Secure Cloud Upload</span>
                    </div>
                  </div>
                )}
      ...
      {isPreviewOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className={darkMode ? 'w-full max-w-3xl border rounded-3xl p-6 shadow-2xl relative space-y-4 animate-in fade-in bg-[#1e293b] border-purple-800/30 text-slate-100' : 'w-full max-w-3xl border rounded-3xl p-6 shadow-2xl relative space-y-4 animate-in fade-in bg-background border-slate-200 text-slate-900'}>
            <div className={darkMode ? 'flex items-center justify-between border-b pb-3 border-purple-800/30' : 'flex items-center justify-between border-b pb-3 border-slate-200'}>
            ...
            <div className={darkMode ? 'p-4 bg-[#0f172a] border border-purple-800/30 rounded-2xl space-y-1.5 text-xs font-mono text-slate-300' : 'p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-1.5 text-xs font-mono text-slate-800'}>
            ...
            {fileMime.includes('pdf') && base64File ? (
              <iframe
                ...
                className={darkMode ? 'w-full h-96 rounded-2xl border border-purple-800/30' : 'w-full h-96 rounded-2xl border border-slate-200'}
              />
            ) : (
              <div className={darkMode ? 'p-8 text-center text-xs text-slate-400 bg-[#0f172a] rounded-2xl border border-purple-800/30' : 'p-8 text-center text-xs text-slate-600 bg-slate-50 rounded-2xl border border-slate-200'}>
                Live document preview available for PDF files. (DOC/DOCX files will be securely uploaded to cloud storage).
